...
@section('content')
  <div class="container mt-4">
    <div class="d-flex justify-content-between mb-3">
      <h2 class="fw-bold">Edit Route Stop</h2>
        ...
        <form action="{{ route('admin.route-stops.update', $routeStop->id) }}" method="POST">
          ...
          <div class="mb-3">
            ...
            <select name="stop_id" id="stop_id" class="form-select @error('stop_id') is-invalid @enderror" required>
              ...
              @foreach($stops as $stop)
                <option value="{{ $stop->id }}" {{ old('stop_id', $routeStop->stop_id) == $stop->id ? 'selected' : '' }}>
          ...
          <div class="mb-3">
            ...
            <input type="number" step="0.01" name="distance_from_origin" id="distance_from_origin" class="form-control @error('distance_from_origin') is-invalid @enderror"
                   value="{{ old('distance_from_origin', $routeStop->distance_from_origin) }}" required>
          ...
          <div class="mb-3">
            ...
            <input type="number" name="estimated_time_min" id="estimated_time_min" class="form-control @error('estimated_time_min') is-invalid @enderror"
                   value="{{ old('estimated_time_min', $routeStop->estimated_time_min) }}" required>
            @error('estimated_time_min') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>

          <div class="form-check mb-3">
            <input type="checkbox" name="is_active" id="is_active" class="form-check-input" {{ old('is_active', $routeStop->is_active) ? 'checked' : '' }}>
            <label for="is_active" class="form-check-label">Active</label>
          </div>

          <div class="d-flex justify-content-end">
            <button type="submit" class="btn btn-primary px-4">
              <i class="bi bi-save"></i> Update Route Stop
            </button>
          </div>
        </form>
      </div>
