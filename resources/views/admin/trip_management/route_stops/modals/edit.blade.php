<!-- resources/views/admin/trip_management/route_stops/modals/edit.blade.php -->
        ...
        <form action="{{ route('admin.route-stops.update', $stop->id) }}" method="POST">
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <select name="route_id" id="route_id{{ $stop->id }}" class="form-select" required>
            ...
            <div class="mb-3">
              ...
              <select name="stop_id" id="stop_id{{ $stop->id }}" class="form-select" required>
                @foreach($stops as $s)
            ...
            <div class="mb-3">
              <label for="stop_order{{ $stop->id }}" class="form-label">Order</label>
              <input type="number" name="stop_order" id="stop_order{{ $stop->id }}" class="form-control" value="{{ $stop->stop_order }}" required>
            </div>

            <div class="mb-3">
              <label for="distance_from_origin{{ $stop->id }}" class="form-label">Distance from Origin (km)</label>
              <input type="number" step="0.01" name="distance_from_origin" id="distance_from_origin{{ $stop->id }}" class="form-control" value="{{ $stop->distance_from_origin }}" required>
            </div>

            <div class="mb-3">
              <label for="estimated_time_min{{ $stop->id }}" class="form-label">Estimated Time (min)</label>
              <input type="number" name="estimated_time_min" id="estimated_time_min{{ $stop->id }}" class="form-control" value="{{ $stop->estimated_time_min }}" required>
            </div>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary">Update Route Stop</button>
          </div>
        </form>
      </div>
    </div>
