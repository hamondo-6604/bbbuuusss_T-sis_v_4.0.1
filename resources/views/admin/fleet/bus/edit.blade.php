          ...
          <div class="mb-3">
            ...
            <select name="seat_layout_id" id="layout_id" class="form-select @error('layout_id') is-invalid @enderror" required>
              ...
              @foreach($seatLayouts as $layout)
                <option value="{{ $layout->id }}" {{ old('layout_id', $bus->layout_id) == $layout->id ? 'selected' : '' }}>
                  {{ $layout->layout_name }}
                </option>
              @endforeach
            </select>
              ...
              @foreach($amenities as $amenity)
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" name="amenities[]" value="{{ $amenity->id }}"
                         id="amenity_{{ $amenity->id }}" {{ in_array($amenity->id, old('amenities', $bus->amenities->pluck('id')->toArray())) ? 'checked' : '' }}>
